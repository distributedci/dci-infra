...
def test_dci_feeder_api_local_settings(host):
    ...
    assert f.contains("^DCI_REPO_RSYNC_TARGET = 'dci-repo.example.com:/data/dci_repo'$")
    assert f.contains("^DCI_REGISTRY_USER = 'reguser'$")
    assert f.contains("^DCI_REGISTRY_PASSWORD = 'regpasswd'$")
